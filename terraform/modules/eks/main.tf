# EKS Module
# Creates an EKS cluster with managed node group and AWS Load Balancer Controller
...
# external-dns IAM Role (IRSA) - reconciles Route53 records from Ingress hosts.
# Scoped to the cluster service account kube-system:external-dns.
...
# Allow record changes on every hosted zone in the account so additional
# applications/domains on this shared cluster need no further IAM changes.
...
# EBS CSI Driver - required for dynamic PersistentVolume provisioning (e.g. Postgres)
